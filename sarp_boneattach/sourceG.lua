...
function attachElementToBone(element, ped, bone, x, y, z, rx, ry, rz)
	if (not (isElement(element) and isElement(ped))) or (getElementType(ped) ~= "ped" and getElementType(ped) ~= "player") then
		return false
	end
	...
	if isFromServerSide then
		triggerClientEvent("attachElementToBone", getRootElement(), element, ped, bone, x, y, z, rx, ry, rz)
	end
...
function detachElementFromBone(element)
	if not element or not attached_ped[element] then
		return false
	end
	...
	if isFromServerSide then
		triggerClientEvent("detachElementFromBone", getRootElement(), element)
	end
...
if not isFromServerSide then
	function getBonePositionAndRotation(ped, bone)
		...
		if not isElement(ped) or (getElementType(ped) ~= "player" and getElementType(ped) ~= "ped") or not isElementStreamedIn(ped) then
			return false
		end
...
addEventHandler(isFromServerSide and "onElementDestroy" or "onClientElementDestroy", getRootElement(), forgetDestroyedElements)

function forgetNonExistingPeds()
	local processedAttachThread = 0
	...
	while true do
		for element, ped in pairs(attached_ped) do
			...
			processedAttachThread = processedAttachThread + 1
			
			if processedAttachThread >= 1000 then
				coroutine.yield()
				processedAttachThread = 0
			end
		end
		coroutine.yield()
	end
end
clearingNonExistingPeds = coroutine.create(forgetNonExistingPeds)
setTimer(function()	coroutine.resume(clearingNonExistingPeds) end, 1000, 0)